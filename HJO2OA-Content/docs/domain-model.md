...
- 正文、摘要快照、索引文本建议与主表拆分，正文正文体可采用 `NVARCHAR(MAX)`、JSON 文本或独立大字段表。
- 一期全文检索建议采用 SQL Server Full-Text Search；二期可按 ADR-004 演进到 Elasticsearch。
- `visibleScope`、`changedFields`、`bodyRef`、`推荐候选` 等结构化字段建议采用 JSON 文本存储。
